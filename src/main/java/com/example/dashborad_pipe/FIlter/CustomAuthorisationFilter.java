...
public class CustomAuthorisationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if(request.getServletPath().equals("/login") || request.getServletPath().equals("/refreshToken") ||request.getServletPath().equals("/video/**") || request.getServletPath().equals("/uploadFile")|| request.getServletPath().equals("/images/**")){
            ...
            if(authorisqtionHeader != null && authorisqtionHeader.startsWith("Bearer ")){
                try{
                    System.out.println(authorisqtionHeader);
                    String token = authorisqtionHeader.substring("Bearer ".length());
